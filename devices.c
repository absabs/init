...
static struct perms_ devperms[] = {
    ...
    { "/dev/tty0",          0660,   AID_ROOT,       AID_ROOT,       0 },
    { "/dev/input/",        0660,   AID_ROOT,       AID_ROOT,      1 },
    ...
    { "/dev/ppp",           0660,   AID_ROOT,       AID_ROOT,       0 },
    { NULL, 0, 0, 0, 0 },
};
...
#else
static void do_coldboot(int event_fd, const char *path)
{
    struct dirent *de;
    char dir_path[128];
    int dfd, fd;
    ...
    snprintf(dir_path, sizeof(dir_path), "%s/%s", path, "uevent");
    ...
    while((de = readdir(d))) {
        ...
        snprintf(dir_path, sizeof(dir_path), "%s/%s", path, de->d_name);
        d2 = opendir(dir_path);
        if (d2) {
            closedir(d2);
            do_coldboot(event_fd, dir_path);
        }
    }
    closedir(d);
}
